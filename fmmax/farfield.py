...
def farfield_profile(
    ...
    brillouin_zone_axes = _absolute_axes(brillouin_zone_axes, flux.ndim)
...
def unflatten_flux(
    ...
    assert flux.ndim >= 4
    assert flux.shape[-2] == 2 * expansion.num_terms
    brillouin_grid_axes = _absolute_axes(brillouin_grid_axes, flux.ndim)
...
def unflatten_transverse_wavevectors(
    ...
    brillouin_grid_axes = _absolute_axes(
...
def _absolute_axes(axes: Tuple[int, ...], ndim: int) -> Tuple[int, ...]:
