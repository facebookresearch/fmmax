...
from fmmax import basis, fft, vector
...
def fourier_matrices_patterned_isotropic_media(
    ...
    if formulation == Formulation.FFT:
        transverse_permittivity_matrix = _transverse_permittivityfft(
...
def _transverse_permittivityfft(
    permittivity: jnp.ndarray,
    expansion: basis.Expansion,
) -> jnp.ndarray:
...
def fourier_matrices_patterned_anisotropic_media(
    ...
    if formulation is Formulation.FFT:
        _transverse_permittivity_fn = _transverse_permittivityfft_anisotropic
        _transverse_permeability_fn = _transverse_permeabilityfft_anisotropic
    else:
        vector_fn = vector.VECTOR_FIELD_SCHEMES[formulation.value]
...
def _transverse_permittivityfft_anisotropic(
...
def _transverse_permeabilityfft_anisotropic(
